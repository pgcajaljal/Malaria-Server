...
static void print_cocos(FILE *fp, char *image_path, detection *dets, int num_boxes, int classes, int w, int h)
{
    ...
    for (i = 0; i < num_boxes; ++i) {
        ...
        for (j = 0; j < classes; ++j) {
            if (dets[i].prob[j]) printf(fp, "{\"image_id\":%d, \"category_id\":%d, \"bbox\":[%f, %f, %f, %f], \"score\":%f},\n", image_id, coco_ids[j], bx, by, bw, bh, dets[i].prob[j]);
        }
    }
}
...
void print_detector_detections(FILE **fps, char *id, detection *dets, int total, int classes, int w, int h)
{
    ...
    for (i = 0; i < total; ++i) {
        ...
        for (j = 0; j < classes; ++j) {
            if (dets[i].prob[j]) printf(fps[j], "%s %f %f %f %f %f\n", id, dets[i].prob[j],
                xmin, ymin, xmax, ymax);
        }
    }
}
...
void print_imagenet_detections(FILE *fp, int id, detection *dets, int total, int classes, int w, int h)
{
    ...
    for (i = 0; i < total; ++i) {
        ...
        for (j = 0; j < classes; ++j) {
            int class = j;
            if (dets[i].prob[class]) printf(fp, "%d %d %f %f %f %f %f\n", id, j + 1, dets[i].prob[class],
                xmin, ymin, xmax, ymax);
        }
    }
}
...
void validate_detector(char *datacfg, char *cfgfile, char *weightfile, char *outfile)
{
    ...
    printf(stderr, "Learning Rate: %g, Momentum: %g, Decay: %g\n", net.learning_rate, net.momentum, net.decay);
    srand(time(0));
    ...
    printf(stderr, "Total Detection Time: %f Seconds\n", (double)time(0) - start);
}
...
void validate_detector_recall(char *datacfg, char *cfgfile, char *weightfile)
{
    ...
    for (i = 0; i < m; ++i) {
        ...
        //printf(stderr, " %s - %s - ", paths[i], labelpath);
        printf(stderr, "%5d %5d %5d\tRPs/Img: %.2f\tIOU: %.2f%%\tRecall:%.2f%%\n", i, correct, total, (float)proposals / (i + 1), avg_iou * 100 / total, 100.*correct / total);
        free(id);
        free_image(orig);
        free_image(sized);
    }
...
void run_detector(int argc, char **argv)
{
    ...
    if (argc < 4) {
        printf(stderr, "usage: %s %s [train/test/valid/demo/map] [data] [cfg] [weights (optional)]\n", argv[0], argv[1]);
        return;
    }
