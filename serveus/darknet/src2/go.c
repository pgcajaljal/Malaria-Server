...
void print_board(float *board, int swap, int *indexes)
{
    ...
    printf(stream, "   ");
    for(i = 0; i < 19; ++i){
        printf(stream, "%c ", 'A' + i + 1*(i > 7 && noi));
    }
    printf(stream, "\n");
    for(j = 0; j < 19; ++j){
        printf(stream, "%2d", (inverted) ? 19-j : j+1);
        for(i = 0; i < 19; ++i){
            int index = j*19 + i;
            if(indexes){
                int found = 0;
                for(n = 0; n < nind; ++n){
                    if(index == indexes[n]){
                        found = 1;
                        /*
                        if(n == 0) printf(stream, "\uff11");
                        else if(n == 1) printf(stream, "\uff12");
                        else if(n == 2) printf(stream, "\uff13");
                        else if(n == 3) printf(stream, "\uff14");
                        else if(n == 4) printf(stream, "\uff15");
                        */
                        if(n == 0) printf(stream, " 1");
                        else if(n == 1) printf(stream, " 2");
                        else if(n == 2) printf(stream, " 3");
                        else if(n == 3) printf(stream, " 4");
                        else if(n == 4) printf(stream, " 5");
                    }
                }
                if(found) continue;
            }
            //if(board[index]*-swap > 0) printf(stream, "\u25C9 ");
            //else if(board[index]*-swap < 0) printf(stream, "\u25EF ");
            if(board[index]*-swap > 0) printf(stream, " O");
            else if(board[index]*-swap < 0) printf(stream, " X");
            else printf(stream, "  ");
        }
        printf(stream, "\n");
    }
    ...
    while(1){
        ...
        if (!strcmp(buff, "protocol_version")){
            ...
            if(type[0] == 'd' || type[0] == 'D'){
                ...
                int i, j;
                int count = 2;
                printf(f, "boardsize 19\n");
                printf(f, "clear_board\n");
                for(j = 0; j < 19; ++j){
                    for(i = 0; i < 19; ++i){
                        if(board[j*19 + i] == 1) printf(f, "play black %c%d\n", 'A'+i+(i>=8), 19-j);
                        if(board[j*19 + i] == -1) printf(f, "play white %c%d\n", 'A'+i+(i>=8), 19-j);
                        if(board[j*19 + i]) ++count;
                    }
                }
                printf(f, "final_status_list dead\n");
                fclose(f);                
#ifdef _WIN32
...
void self_go(char *filename, char *weightfile, char *f2, char *w2, int multi)
{
    ...
    while(1){
        if (done || count >= 300){
            ...
            ++total;
            printf(stderr, "Total: %d, Player 1: %f, Player 2: %f\n", total, (float)p1/total, (float)p2/total);
            int j;
            for(; i < count; i += 2){
                for(j = 0; j < 93; ++j){
                    printf("%c", boards[i][j]);
...
void run_go(int argc, char **argv)
{
    //boards_go();
    if(argc < 4){
        printf(stderr, "usage: %s %s [train/test/valid] [cfg] [weights (optional)]\n", argv[0], argv[1]);
        return;
    }
