          ...
          if (print_errors)
            {
              ...
              free (buf);
#else
              printf (stderr, _("%s: option `%s' is ambiguous\n"),
                       argv[0], argv[optind]);
#endif
            }
              ...
              if (argv[optind][1] == '-')
                {
                  ...
                  __asprintf (&buf, _("%s: unrecognized option `--%s'\n"),
                              argv[0], nextchar);
#else
                  printf (stderr, _("%s: unrecognized option `--%s'\n"),
                           argv[0], nextchar);
#endif
                }
              else
...
#else
                  printf (stderr, _("%s: unrecognized option `%c%s'\n"),
                           argv[0], argv[optind][0], nextchar);
#endif
                }
            ...
            if (posixly_correct)
              {
                ...
                __asprintf (&buf, _("%s: illegal option -- %c\n"),
                            argv[0], c);
#else
                printf (stderr, _("%s: illegal option -- %c\n"), argv[0], c);
#endif
              }
            else
              {
                ...
                __asprintf (&buf, _("%s: invalid option -- %c\n"),
                            argv[0], c);
#else
                printf (stderr, _("%s: invalid option -- %c\n"), argv[0], c);
#endif
              }
            ...
            if (print_errors)
              {
                ...
                free (buf);
#else
                printf (stderr, _("%s: option requires an argument -- %c\n"),
                         argv[0], c);
#endif
              }
            optopt = c;
            ...
            if (print_errors)
              {
                ...
                free (buf);
#else
                printf (stderr, _("%s: option `-W %s' is ambiguous\n"),
                         argv[0], argv[optind]);
#endif
              }
            nextchar += strlen (nextchar);
