    ...
    while(get_current_batch(net) < net.max_batches || net.max_batches == 0){
        ...
        if (i % 10 == 0) {
            if (calc_topk) {
                ...
                if (topk > 0) printf(stderr, " Last accuracy TOP5 = %2.2f %% \n", topk * 100);
            }

            if (net.cudnn_half) {
                if (i < net.burn_in * 3) printf(stderr, " Tensor Cores are disabled until the first %d iterations are reached.\n", 3 * net.burn_in);
                else printf(stderr, " Tensor Cores are used.\n");
            }
        }
...
void test_classifier(char *datacfg, char *cfgfile, char *weightfile, int target_layer)
{
    ...
    for(curr = net.batch; curr < m; curr += net.batch){
        ...
        printf(stderr, "Loaded: %d images in %lf seconds\n", val.X.rows, sec(clock()-time));
        ...
        printf(stderr, "%lf seconds, %d images, %d total\n", sec(clock()-time), val.X.rows, curr);
        free_data(val);
    }
}
