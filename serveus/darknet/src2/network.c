...
int resize_network(network *net, int w, int h)
{
    ...
    int inputs = 0;
    size_t workspace_size = 0;
    //printf(stderr, "Resizing to %d x %d...\n", w, h);
    //fflush(stderr);
    for (i = 0; i < net->n; ++i){
        layer l = net->layers[i];
        //printf(" %d: layer = %d,", i, l.type);
...
detection_layer get_network_detection_layer(network net)
{
    ...
    printf(stderr, "Detection layer not found!!\n");
    detection_layer l = {0};
    return l;
}
...
void print_network(network net)
{
    ...
    for(i = 0; i < net.n; ++i){
        ...
        printf(stderr, "Layer %d - Mean: %f, Variance: %f\n",i,mean, vari);
        if(n > 100) n = 100;
        for(j = 0; j < n; ++j) printf(stderr, "%f, ", output[j]);
        if(n == 100)printf(stderr,".....\n");
        printf(stderr, "\n");
    }
}
