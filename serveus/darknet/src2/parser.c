...
learning_rate_policy get_policy(char *s)
{
    ...
    printf(stderr, "Couldn't find policy %s, going with constant\n", s);
    return CONSTANT;
}
...
void parse_net_options(list *options, network *net)
{
    ...
    if (net->gpu_index >= 0) {
        ...
        printf(stderr, " compute_capability = %d, cudnn_half = %d \n", compute_capability, net->cudnn_half);
    }
    else printf(stderr, " GPU isn't used \n");
#endif
    if(net->policy == STEP){
...
network parse_network_cfg_custom(char *filename, int batch, int time_steps)
{
    ...
    int count = 0;
    free_section(s);
    printf(stderr, "layer     filters    size              input                output\n");
    while(n){
        params.index = count;
        printf(stderr, "%4d ", count);
        s = (section *)n->val;
        options = s->options;
        layer l = {0};
        ...
        if(lt == CONVOLUTIONAL){
            ...
            printf(stderr, "Type not recognized: %s\n", s->type);
        }
    ...
    printf(stderr, "Total BFLOPS %5.3f \n", bflops);
#ifdef GPU
    get_cuda_stream();
    ...
    if (gpu_index >= 0)
    {
        ...
        if (workspace_size) {
            printf(stderr, " Allocate additional workspace_size = %1.2f MB \n", (float)workspace_size/1000000);
        ...
        switch(line[0]){
            ...
            default:
                if(!read_option(line, current->options)){
                    printf(stderr, "Config file error line %d, could parse: %s\n", nu, line);
                    free(line);
                }
                break;
        }
...
#endif
    printf(stderr, "Saving weights to %s\n", filename);
...
#endif
    printf(stderr, "Loading weights from %s...", filename);
    fflush(stdout);
