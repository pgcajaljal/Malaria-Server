...
from serveus.forms import LoginForm, RecoveryForm, ChangePassForm
...
# error import winrandom
# Download from here https://github.com/axper/python3-pycrypto-windows-installer/blob/master/pycrypto-2.6.1.win-amd64-py3.4.exe
...
def maps(disease_id):
	...
	if default_view or not(lat and lng):
		...
		# TODO: Move to client side javascript if applicable (99)
		try:
...
def timeline(disease_id):
	...
	disease_list = Infection.query.all()
	specimen_types = Infection.query.with_entities(Infection.specimen_type).distinct()
	# TODO: REFACTOR THIS WHOLE THING (50)
	# Filter arguments
	lat = request.args.get('lat')
	lng = request.args.get('lng')
	zoom = request.args.get('zoom')
...
def view_log(date, time):
	...
	if date == "latest" or (date == DATE_INIT and time == TIME_INIT):
		...
		date = DATE_INIT
		time = TIME_INIT
s		name = "Latest | Date: %s Time: %s" % (date, time.replace('.',':'))
	else:
...
def upload_chunk():
	if request.method == 'POST':
		...
		if f:
			...
			#TODO: check if folder exists (10)
			#TODO: Refactor code and comments (20)
...
def retype():
	if request.method == 'POST':
		...
		private_key = RSA.importKey(Key.query.first().private_key) #TODO: own key (99)
...
def update_validation():
	if request.method == 'POST':
		...
		if filenames:
			...
			# print("THIS IS THE JSON.DUMPS(RET)")
